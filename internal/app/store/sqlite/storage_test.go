...
func Test_SqliteStorage(t *testing.T) {
	...
	var err error
	store, err := NewStorage(ctx, "file:test.db?cache=shared&mode=rwc")
	if err != nil {
...
func Test_SqliteStorage_View(t *testing.T) {
	...
	var err error
	store, err := NewStorage(ctx, "file:test_view.db?cache=shared&mode=rwc")
	if err != nil {
		log.Printf("[ERROR] Failed to open SQLite storage: %e", err)
	}
...
func Test_SqliteStorage_readOnly(t *testing.T) {
	...
	var err error
	store, err := NewStorage(ctx, "file:test_ro.db?cache=shared&mode=rc")
	if err != nil {
		log.Printf("[ERROR] Failed to open SQLite storage: %e", err)
	}
